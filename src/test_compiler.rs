    ...
    for (key, value) in map{
        if value.is_object(){
            if flag{
                ...
                output.push(Types::String(key.clone()));
                ...
                output.push(Types::String(hash.clone()));
                let newmap: HashMap<String, Value> = from_value(value.clone())?;
            ...
            for item in array{
                if item.is_object(){
                    ...
                    sub_array.push(Types::String(item.to_string()));
                }
            }
            // if !obj{
            //     output_string.pop();
            //     output_string = output_string+"],";
            // }   
            output.push(Types::Array(sub_array));
        }
        else {
            // output_string = output_string+&value.to_string()+",";
            output.push(Types::String(value.to_string()));
        }
    }
    // println!("{:?}", rules);
